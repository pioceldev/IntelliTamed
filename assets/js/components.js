/* ============================================================
   IntelliTamed — Composants partagés
   Sidebar, topbar, footer, icônes SVG (injection JS, compatible file://)
   ============================================================ */

(function (global) {
  "use strict";

  /* ---------- Icônes SVG (stroke = couleur courante) ---------- */
  var ICONS = {
    ...
    actionPlan: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>',
    analysis: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 14l4-4 3 3 5-6"/></svg>',
    opportunities: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>',
    objectives: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1.5"/></svg>',
    admin: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>',
    ...
    sparkles: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l1.9 5.1L19 10l-5.1 1.9L12 17l-1.9-5.1L5 10l5.1-1.9z"/><path d="M19 15l.9 2.1L22 18l-2.1.9L19 21l-.9-2.1L16 18l2.1-.9z"/></svg>',
    ...
    building: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="9" y1="6" x2="15" y2="6"/><line x1="9" y1="10" x2="15" y2="10"/><line x1="9" y1="14" x2="15" y2="14"/></svg>',
  ...
  var NAV = [
    ...
    { id: "assistant", label: "Assistant IA", icon: "assistant", href: "assistant.html" },
    { id: "projects", label: "Mes Projets", icon: "projects", href: "projects.html" },
  ...
  var NAV_BOTTOM = [
    { id: "admin", label: "Administration", icon: "admin", href: "admin.html" },
    { id: "profile", label: "Paramètres", icon: "settings", href: "profile.html" }
  ];
  ...
  function icon(name, cls) {
    ...
    return "<span" + c + " aria-hidden=\"true\">" + (ICONS[name] || "") + "</span>";
  }
  ...
  function sidebarHTML(activeId, opts) {
    ...
    return '' +
      '<aside class="sidebar" id="sidebar" aria-label="Navigation principale">' +
        ...
        "</div>" +
      "</aside>" +
      '<div class="sidebar-overlay" data-close-sidebar hidden></div>';
  }
    ...
    return '' +
      '<header class="topbar">' +
        '<div class="topbar-left">' +
          '<button class="btn-icon sidebar-toggle" type="button" data-open-sidebar aria-label="Ouvrir le menu">' + icon("menu") + "</button>" +
          '<div class="breadcrumb"><span>' + esc(breadcrumb) + "</span>" +
            (badge ? '<span class="chip chip-blue">' + icon("sparkles", "chip-icon") + esc(badge) + "</span>" : "") +
          "</div>" +
        "</div>" +
        '<div class="topbar-right">' +
          '<label class="topbar-search">' + icon("search") +
  ...
  function appFooterHTML(cfg) {
    ...
    return '<footer class="app-footer">' +
      ...
      '<nav>' +
        ...
        '<a href="#" data-toast="Support disponible 24h/24 via l’assistant IA.">Support</a>' +
        '<a href="#" data-toast="Politique de confidentialité.">Confidentialité</a>' +
      "</nav>" +
      "</footer>";
  }
  ...
  function injectAll() {
    document.querySelectorAll("[data-inject]").forEach(function (el) {
      ...
      if (type === "sidebar") {
        ...
        el.outerHTML = topbarHTML(cfg);
      } else if (type === "app-footer") {
        el.outerHTML = appFooterHTML({
          left: el.getAttribute("data-left") || undefined,
          extra: el.getAttribute("data-extra") || undefined
        });
      }
    });
  }
